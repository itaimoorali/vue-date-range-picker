...
import {RANGE_VALUE_MAP, TAB_NAMES, RANGE_LABEL_MAP} from './date-picker-config';
...
export function formatLabel({activeTab, selectedRange, selectedRangeOption}) {
  ...
  if (isStartInCurrentYear && isEndInCurrentYear) {
    if (isInSameMonth) {
      return `${format(startRangeDate, 'MMM dd')} - ${format(endRangeDate, 'dd')}`;
    }
    return `${format(startRangeDate, 'MMM dd')} ~ ${format(endRangeDate, 'MMM dd')}`;
  }

  if (isStartInCurrentYear && !isEndInCurrentYear) {
    return `${format(startRangeDate, 'MMM dd')} ~ ${format(endRangeDate, 'MMM dd, yyyy')}`;
  }

  if (!isStartInCurrentYear && isEndInCurrentYear) {
    return `${format(startRangeDate, 'MMM dd, yyyy')} ~ ${format(endRangeDate, 'MMM dd')}`;
  }

  if (!isInSameYear) {
    return `${format(startRangeDate, 'MMM dd, yyyy')} ~ ${format(endRangeDate, 'MMM dd, yyyy')}`;
  }

  if (isInSameYear) {
    if (isInSameMonth) {
      return `${format(startRangeDate, 'MMM dd')} - ${format(endRangeDate, 'dd, yyyy')}`;
    }
